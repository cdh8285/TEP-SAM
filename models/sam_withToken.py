...
def _iou_loss(pred, target):
    ...
    iou = 1 - inter / (union + np.spacing(1))
...
def _dice_loss(pred, target):
    ...
    sums = prob.sum(dim=(2,3)) + target.sum(dim=(2,3))
    dice = (2*inter + eps) / (sums + eps)
    return (1.0 - dice).mean()

def _compute_iou_batch(pred, gt, eps=1e-6):
        ...
        pmin = float(pred_t.min())
        pmax = float(pred_t.max())
        if pmin < -0.01 or pmax > 1.01:
            pred_prob = torch.sigmoid(pred_t)
        else:
            pred_prob = pred_t

        B = pred_prob.size(0)
        pred_flat = pred_prob.view(B, -1)
        gt_flat = gt_t.view(B, -1).float()
...
class SAM(nn.Module):
    ...
    def set_input(self, input, single_input ,gt_mask, gt_middle):
        self.input = input.to(self.device)
        self.single_input = single_input
        self.gt_mask = gt_mask.to(self.device)
        self.gt_middle = gt_middle.to(self.device)
    ...
    def forward(self):
        ...
        self.features, self.temporal_token_embedding, self.middle_mask = self.image_encoder(self.input)

        sparse_embeddings_list = []
        iou_predictions_list = []
        masks_list = []
        sparse_embeddings_empty = torch.empty((bs, 0, self.prompt_embed_dim), device=self.input.device)
        ...
        low_res_masks, iou_predictions = self.mask_decoder(
            ...
            temporal_token_embedding=self.temporal_token_embedding,
            image_pe=self.get_dense_pe(),
            sparse_prompt_embeddings=sparse_embeddings_empty,
        ...
        self.pred_mask = torch.cat(masks_list, dim=0)
    ...
    def infer(
        ...
        """
        Single- or multi-image inference with optional point/box prompts.
        Args:
        input:             Tensor of shape (B, 3, H, W).
        point_coords:      Optional Tensor of shape (B, N, 2).
        point_labels:      Optional Tensor of shape (B, N).
        boxes:             Optional Tensor of shape (B, 4).

        Returns:
        masks: Tensor of shape (B, 1, H, W), the predicted binary masks.
        """
        bs = input.shape[0]

        sparse_embeddings = torch.empty((bs, 0, self.prompt_embed_dim), device=input.device)

        features, temporal_token_embedding, _ = self.image_encoder(input)
        ...
        low_res_masks, _ = self.mask_decoder(
            ...
            temporal_token_embedding=temporal_token_embedding,
            image_pe=self.get_dense_pe(),
    ...
    def backward_G(self):
        ...
        self.loss_G += _dice_loss(self.pred_mask, self.gt_mask)

        alpha = 1

        use_deep_supervise = True
        if use_deep_supervise:
            self.loss_G += _dice_loss(self.middle_mask, self.gt_middle) * alpha

            self.loss_G += self.criterionBCE(self.middle_mask, self.gt_middle) * alpha

        use_token = False

        if use_token:
            gt_iou_target = _compute_iou_batch(self.pred_mask, self.gt_mask).detach()
            iou_pred = self.iou_predictions.view(-1)
            loss_iou = F.mse_loss(iou_pred, gt_iou_target, reduction='mean')
            self.loss_G += 0.1 * loss_iou
