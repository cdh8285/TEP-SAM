...
def make_data_loader(spec, tag=''):
    ...
    sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=(tag != 'val'))
...
def train(train_loader, model, epoch):
    ...
    for batch in train_loader:
        for k, v in batch.items():
            batch[k] = v.to(device)
        inp = batch['inp']
        single_inp = batch['single_inp']
        gt = batch['gt']
        gt_middle = batch['gt_middle']
        model.set_input(inp, single_inp, gt, gt_middle)
    ...
    avg_loss = mean([l.item() for l in loss_list])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='configs/sam-vit-b-token-NUDT-MIRSDT.yaml')
    parser.add_argument('--name',
                        default='tep-sam-sam1')
    ...
    save_path = os.path.join('./weights', save_name)
